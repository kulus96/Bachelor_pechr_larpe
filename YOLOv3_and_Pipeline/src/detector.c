...
float validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net)
{
    ...
    recall_comp = fopen("recall_comp.txt",'a');
    fprintf(recall_comp,"%d %d %1.2f %2.2f", tp_for_thresh, fp_for_thresh,cur_recall,avg_iou*100);
    fclose(recall_comp);
    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    ...
    while (1) {
        ...
        //getROI(im,dets,nboxes, thresh,1); //set 1 to 0 if images are not wished to be saved
        draw_detections_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output);

        //estimate_depth(dets,thresh);
...
{   
    ...
    if (0 == strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, outfile, save_labels, letter_box, benchmark_layers);
    else if (0 == strcmp(argv[2], "runYolov3")) setupDetectorPy(datacfg, cfg, weights, thresh,  hier_thresh,  dont_show,  ext_output,  save_labels, outfile,  letter_box,  benchmark_layers);
