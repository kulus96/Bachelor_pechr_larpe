...
image **alphabet;
int count;
int benchmark_var, benchmark_layer, prefixx, frameSkip, delay, time_limSec;
pthread_t fetch_thread;
pthread_t detect_thread;
layer l;
...
void run_detectorPy(int dont_show)
{
    ...
    //for(int i = 0; i < 1 ; i++){ //while(1)
        ++count;
        {
            ...
            if (!benchmark_var) if (pthread_create(&fetch_thread, 0, fetch_in_thread, 0)) error("Thread creation failed");
            if(pthread_create(&detect_thread, 0, detect_in_thread, 0)) error("Thread creation failed");
            ...
            if (!benchmark_var) draw_detections_cv_v3(show_img, local_dets, local_nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output);
            free_detections(local_dets, local_nboxes);
            ...
            if(!prefixx){
                ...
                char buff[256];
                sprintf(buff, "%s_%08d.jpg", prefixx, count);
                if(show_img) save_cv_jpg(show_img, buff);
            }
            ...
            if (time_limSec > 0 && (get_time_point() - start_time_lim)/1000000 > time_limSec) {
                printf(" start_time_lim = %f, get_time_point() = %f, time spent = %f \n", start_time_lim, get_time_point(), get_time_point() - start_time_lim);
                return;//break;
            }

            if (flag_exit == 1) return;//break;

            if(delay == 0){
                if(!benchmark_var) release_mat(&show_img);
                show_img = det_img;
            }
            det_img = in_img;
            det_s = in_s;
...
void end_detectorPy()
{
    ...
    free_ptrs((void **)name, net.layers[net.n - 1].classes);
...
void get_bbox(detection *dets, float thresh, int NumberofDetections)
{
